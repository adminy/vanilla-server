...
struct FEmpireChatPacket
{
	...
	void operator () (LPDESC d)
	{
		...
		else
		{
			...
			size_t len = sizeof(converted_msg);
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		if(global_chat)
		{
			...
			if(char_empire == 1)
			{
				strncpy(buf, LC_TEXT("Shinsoo"), sizeof(buf));
				...
				strncpy(buf, LC_TEXT("Chunjo"), sizeof(buf));
				...
				strncpy(buf, LC_TEXT("Jinno"), sizeof(buf));
			...
			strncpy(p.szText, chatbuf_global, sizeof(p.szText));
